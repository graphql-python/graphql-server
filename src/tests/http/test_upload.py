...
async def test_upload(enabled_http_client: HttpClient):
    ...
    assert response.json["data"] == {"readText": "graphql_server"}


async def test_file_list_upload(enabled_http_client: HttpClient):
    query = "mutation($files: [Upload!]!) { readFiles(files: $files) }"
    file1 = BytesIO(b"graphql_server1")
    file2 = BytesIO(b"graphql_server2")
    ...
    assert data["readFiles"][0] == "graphql_server1"
    assert data["readFiles"][1] == "graphql_server2"
...
async def test_nested_file_list(enabled_http_client: HttpClient):
    ...
    assert data["readFolder"][0] == "graphql_server1"
    assert data["readFolder"][1] == "graphql_server2"
...
async def test_upload_single_and_list_file_together(enabled_http_client: HttpClient):
    ...
    file1 = BytesIO(b"graphql_server1")
    file2 = BytesIO(b"graphql_server2")
    file3 = BytesIO(b"graphql_server3")
    ...
    assert data["readFiles"][0] == "graphql_server1"
    assert data["readFiles"][1] == "graphql_server2"
    assert data["readText"] == "graphql_server3"
