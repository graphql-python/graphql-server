...
@mock.patch("graphql_server.channels.router.GraphQLHTTPConsumer.as_asgi")
@mock.patch("graphql_server.channels.router.GraphQLWSConsumer.as_asgi")
@pytest.mark.parametrize("pattern", ["^graphql", "^foo"])
def test_included_paths(ws_asgi: mock.Mock, http_asgi: mock.Mock, pattern: str):
    from graphql_server.channels.router import GraphQLProtocolTypeRouter
