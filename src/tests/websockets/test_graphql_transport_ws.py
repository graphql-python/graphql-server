...
from graphql_server import runtime as graphql_server_runtime
...
async def test_subscription_field_errors(ws: WebSocketClient):
    process_errors = Mock()
    with patch.object(graphql_server_runtime, "process_errors", process_errors):
        await ws.send_message(
            {
                "id": "sub1",
                "type": "subscribe",
...
async def test_single_result_mutation_operation(ws: WebSocketClient):
    ...
    next_message: NextMessage = await ws.receive_json()
    assert_next(next_message, "sub1", {"hello": "teststring"})
...
@pytest.mark.parametrize(
    ("extra_payload", "expected_message"),
    [
        # TODO: INCOMPATIBLE WITH GRAPHQL-CORE
        # ({}, "Hello graphql-server1"),
        # ({"operationName": None}, "Hello graphql-server1"),
        ({"operationName": "Query1"}, "Hello graphql-server1"),
        ({"operationName": "Query2"}, "Hello graphql-server2"),
    ],
)
async def test_single_result_operation_selection(
    ...
    query = """
        query Query1 {
            hello(name: "graphql-server1")
        }
        query Query2 {
            hello(name: "graphql-server2")
        }
    """
...
async def test_injects_connection_params(ws_raw: WebSocketClient):
    ...
    next_message: NextMessage = await ws.receive_json()
    assert_next(next_message, "sub1", {"connectionParams": {"graphql_server": "rocks"}})
...
async def test_subscription_errors_continue(ws: WebSocketClient):
    ...
    with patch.object(graphql_server_runtime, "process_errors", process_errors):
        ...
        # TODO: INCOMPATIBLE WITH OTHER TESTS
        assert next_message2["payload"]["data"] == {"flavorsInvalid": None}
