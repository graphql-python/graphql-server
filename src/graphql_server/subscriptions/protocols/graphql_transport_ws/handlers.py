...
from graphql_server.utils.operation import get_operation_type
...
class BaseGraphQLTransportWSHandler(Generic[Context, RootValue]):
    task_logger: logging.Logger = logging.getLogger("graphql_server.ws.task")
    ...
    async def handle_connection_init(self, message: ConnectionInitMessage) -> None:
        ...
        await self.view.setup_connection_params(
            payload,
    ...
    async def handle_subscribe(self, message: SubscribeMessage) -> None:
        ...
        try:
            ...
            if operation_name is None:
                e = "Can't get GraphQL operation type"
            else:
                e = f'Unknown operation named "{operation_name}".'
    ...
    async def run_operation(self, operation: Operation[Context, RootValue]) -> None:
        ...
        result_source: ExecutionResult | AsyncGenerator[ExecutionResult, None]
        ...
        try:
            ...
            if isinstance(result_source, ExecutionResult):
                ...
                async for result in result_source:
                    if (
                        is_first_result
                        and result.errors
                        and isinstance(result.errors[0], GraphQLValidationError)
                    ):
            ...
            await operation.send_initial_errors(e.errors)
