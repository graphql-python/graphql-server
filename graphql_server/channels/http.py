...
from ..render_graphiql import render_graphiql_sync
from .. import GraphQLParams
...
class GraphQLHttpConsumer(AsyncHttpConsumer):
    ...
    async def get(self, body):
        if self.should_render_graphiql():
            return await self.render_graphiql(body, params=GraphQLParams(query=""))
        else:
            await self.send_response(
