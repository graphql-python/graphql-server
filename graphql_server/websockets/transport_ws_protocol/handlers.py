...
from typing import Any, AsyncGenerator, Dict, Optional, TypedDict, cast
...
class BaseGraphQLTransportWSHandler(ABC):
    ...
    async def handle_message(self, message: dict):
        try:
            ...
            if message_type == ConnectionInitMessage.type:
                await self.handle_connection_init(cast(ConnectionInitMessage, message))

            elif message_type == PingMessage.type:
                await self.handle_ping(cast(PingMessage, message))

            elif message_type == PongMessage.type:
                await self.handle_pong(cast(PongMessage, message))

            elif message_type == SubscribeMessage.type:
                await self.handle_subscribe(cast(SubscribeMessage, message))

            elif message_type == CompleteMessage.type:
                await self.handle_complete(cast(CompleteMessage, message))
    ...
    async def handle_subscribe(self, message: SubscribeMessage) -> None:
        ...
        if isinstance(result_source, GraphQLExecutionResult):
            ...
            await self.send_message(ErrorMessage(id=message.id, payload=payload))
            self.process_errors(result_source.errors)
            return
